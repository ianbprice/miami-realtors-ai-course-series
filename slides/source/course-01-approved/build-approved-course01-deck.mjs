...
const slides = [
  ...
  ['section','SECTION 1','AI Reality Check','AI is useful when it supports work agents already do.'],
  ...
  ['section','SECTION 2','Client Communication','Better context creates messages that sound more like a professional agent.'],
  ...
  ['beforeafter','PROMPT EXAMPLE','Formula in action: client follow-up prompt','Write a follow-up text for a buyer.','You are helping me as a real estate professional. Draft a warm, concise text for a buyer who toured two condos, liked one, but is worried about monthly payment. Avoid financial advice or promises. Ask one clear next-step question.','This slide shows how the formula becomes a usable prompt. Keep the prompt readable and emphasize role, task, context, tone, constraints, and goal.'],
  ...
  ['prompts','PROMPTS','Reusable communication prompts: Part 1',['Rewrite this text so it sounds warm, concise, and natural for a REALTOR: [draft]','Create a buyer follow-up email based on these call notes: [notes]. Ask one clear next-step question.','Draft a seller update after a quiet week. Be honest, professional, and solution-oriented.'],'Encourage attendees to save prompts that work.'],
  ['prompts','PROMPTS','Reusable communication prompts: Part 2',['Create three versions of this follow-up: text, email, and voicemail.','Review this message for accuracy, Fair Housing risk, and claims I should verify.','Give me three tone options: concise, reassuring, and more direct.'],'Use one chat per client, property, or workflow when context matters. Use separate chats when topics, clients, or confidential contexts should not mix.'],
  ['section','SECTION 3','Lead Follow-Up','Context before cadence: different lead sources need different first responses.'],
  ...
  ['section','SECTION 4','Listings and Marketing','Facts first. Polish second. Human review always.'],
  ...
  ['section','SECTION 5','Daily Workflow and Guardrails','Attach AI to the day, not to a vague intention.'],
...
function courseCue(title) {
  const cues = [
    ...
    ['By the end, AI should feel useful', 'Use it where work repeats', 'Start with messages, notes, follow-up, and prep before chasing new tools.'],
    ...
    ['Before you use an AI tool', 'Private, verified, compliant, reviewed', 'Treat public AI tools carefully and review every output before use.'],
    ['Most follow-up fails', 'Useful beats frequent', 'Better follow-up gives the lead a reason to respond and a clear next step.'],
    ['Start with verified facts', 'Facts first', 'Use verified facts and tell AI not to invent missing details.'],
...
function normalSlide(spec, i) {
  ...
  if (type === 'bullets') {
    bulletText(slide, a, 0.95, 2.05, 7.0, 4.0, t.startsWith('By the end') ? 15.2 : 16.6);
    const cue = courseCue(t);
    spotlightCard(slide, cue.heading, cue.body, i);
  } else if (type === 'agenda') {
    ...
    a.forEach((p, idx) => {
      const x = 0.92 + (idx % 2) * 5.65, y = 1.92 + Math.floor(idx / 2) * 1.28;
      box(slide, x, y, 4.95, 0.92, C.white);
      slide.addText(p[0], { x: x+0.26, y: y+0.15, w: 1.45, h: 0.18, fontSize: 10.8, bold: true, color: C.teal, margin: 0 });
      slide.addText(p[1], { x: x+0.26, y: y+0.44, w: 4.35, h: 0.24, fontSize: 13.0, color: C.ink, margin: 0, fit: 'shrink' });
    });
    ...
    a.forEach((p, idx) => {
      const y = 1.88 + idx * (a.length > 3 ? 0.74 : 0.88);
      const h = a.length > 3 ? 0.58 : 0.68;
      box(slide, 0.95, y, 10.75, h, idx % 2 ? C.white : C.sky, idx % 2 ? C.line : 'B8DDE0');
      dot(slide, 1.18, y+0.09, String(idx+1), [C.teal,C.blue,C.coral,C.green,C.gold][idx%5]);
    ...
    if (k === 'FINAL ACTION') {
      ...
      a.forEach((st, idx) => {
        ...
        dot(slide, x+0.22, y+0.28, String(idx+1), [C.teal,C.blue,C.coral,C.green,C.gold,C.navy][idx%6]);
        slide.addText(st, { x: x+0.82, y: y+0.28, w: 2.2, h: 0.42, fontSize: 11.8, bold: true, color: C.ink, margin: 0, fit: 'shrink' });
      });
    } else {
      a.forEach((st, idx) => {
        ...
        slide.addText(st, { x: x-0.15, y: 3.25, w: 1.55, h: 0.7, fontSize: 11.8, bold: true, color: C.ink, align: 'center', margin: 0, fit: 'shrink' });
      });
    }
  } else if (type === 'grid') {
    a.forEach((it, idx) => {
    ...
    slide.addShape(deck.ShapeType.rect, { x: 1.0, y: 1.95, w: 0.14, h: 3.0, fill: { color: C.teal }, line: { color: C.teal } });
    box(slide, 1.2, 1.95, 10.45, 3.0, C.white, 'C9DFE5');
    slide.addText(`"${a}"`, { x: 1.65, y: 2.42, w: 9.35, h: 1.55, fontSize: 17.2, italic: true, color: C.ink, margin: 0.03, fit: 'shrink' });
    slide.addText('Role-play prompt', { x: 1.65, y: 4.34, w: 2.0, h: 0.18, fontSize: 10, bold: true, color: C.teal, margin: 0 });
    ...
    a.forEach((r, idx) => {
      const x = 0.95 + (idx % 2) * 5.55, y = 1.95 + Math.floor(idx / 2) * 1.55;
      box(slide, x, y, 4.95, 1.14, idx % 2 ? C.white : C.sky);
      slide.addText(r[0], { x: x+0.28, y: y+0.2, w: 4.3, h: 0.18, fontSize: 11.2, bold: true, color: C.teal, margin: 0, fit: 'shrink' });
      slide.addText(r[1], { x: x+0.28, y: y+0.52, w: 4.35, h: 0.38, fontSize: 10.7, color: C.ink, margin: 0, fit: 'shrink' });
    });
    ...
    slide.addText('Teaching Focus:', { x: 7.25, y: 2.33, w: 3.3, h: 0.38, fontSize: 22, bold: true, color: C.navy, margin: 0 });
    slide.addText(goHouseBullets[0], { x: 7.25, y: 3.05, w: 3.85, h: 0.82, fontSize: 13.0, color: C.ink, margin: 0.02, fit: 'shrink', breakLine: false });
    slide.addShape(deck.ShapeType.line, { x: 7.25, y: 4.36, w: 1.2, h: 0, line: { color: C.teal, width: 3 } });
    slide.addText(goHouseBullets[1], { x: 7.25, y: 4.72, w: 3.85, h: 0.55, fontSize: 10.5, bold: true, color: C.green, margin: 0.02, fit: 'shrink', breakLine: false });
...
function addSlide(spec, i) {
  ...
  if (type === 'cover') {
    ...
    slide.addShape(deck.ShapeType.roundRect, { x: 7.85, y: 2.35, w: 3.15, h: 2.05, rectRadius: 0.06, fill: { color: '1E4768', transparency: 18 }, line: { color: '5FBDBD', transparency: 45, width: 1 } });
    slide.addText('Practical workshop focus', { x: 8.18, y: 2.72, w: 2.5, h: 0.2, fontSize: 10.5, bold: true, color: C.gold, margin: 0 });
    slide.addText('Client messages\nLead follow-up\nListing prep\nAppointment prep', { x: 8.18, y: 3.05, w: 2.45, h: 0.78, fontSize: 12.2, color: 'D7EDEE', margin: 0.02, breakLine: false, fit: 'shrink' });
